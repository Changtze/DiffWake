...
def create_wake(farm_dict):
    ...
    wake = WakeModelManager(velocity_model = CumulativeGaussCurlVelocityDeficit(**farm_dict['wake']['wake_velocity_parameters']['cc']),
...
def turbine_powers(state: CCState) -> jnp.ndarray:
    return power(
        ...
        yaw_angles=state.yaw_angles,
        tilt_angles=state.tilt_angles,
